...
export const PluelyPrompts = () => {
  ...
  // Premium unlocked — `hasActiveLicense` is always true now, so we no
  // longer need to gate the Pluely default prompts behind a license check.
  ...
  const handleSelectPluelyPrompt = async (prompt: PluelyPrompt) => {
    // Premium unlocked — every Pluely prompt is selectable for free.
    try {
