...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  // The system prompt the AI actually uses. Priority:
  //   1. The currently selected user / Pluely prompt (STORAGE_KEYS.SYSTEM_PROMPT)
  //   2. The user-customizable default (STORAGE_KEYS.CUSTOM_DEFAULT_PROMPT)
  //   3. The hard-coded DEFAULT_SYSTEM_PROMPT
  ...
  // PREMIUM UNLOCKED — every Pluely build now ships with all premium
  // features enabled for free. The license / checkout flow has been
  // disabled at the frontend layer; `hasActiveLicense` is always true
  // so every gated UI (Theme, PluelyPrompts, Generate-with-AI, screenshot
  // auto-mode, etc.) just works out of the box.
  ...
  const getActiveLicenseStatus = async () => {
    // PREMIUM UNLOCKED — skip the remote license validation call entirely.
    // The previous implementation invoked `validate_license_api` against the
    // Pluely payment backend, which would either fail (no API access key
    // configured for this build) or return `is_active: false` for users
    // without a paid license, locking away premium features.
    //
    // We now treat every install as fully licensed. We still notify the
    // Rust side so registered shortcuts and other license-aware backend
    // behavior stay consistent with the unlocked frontend state.
  ...
  const loadData = () => {
    // Load system prompt — fall back to the user-editable default prompt
    // (which itself falls back to the hard-coded DEFAULT_SYSTEM_PROMPT) when
    // no specific prompt has been selected by the user.
    ...
    if (savedSystemPrompt) {
      ...
      setSystemPrompt(getEffectiveDefaultSystemPrompt());
    }
